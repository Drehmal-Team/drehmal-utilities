...
execute store result score #tempdist ntils.z.cast.temp run data get storage ntils:z cast.box[-1][5]
...
execute store result score #az ntils.z.cast.temp run scoreboard players operation #tempdist ntils.z.cast.temp *= #s/16z ntils.z.cast.temp

scoreboard players operation #tempdist ntils.z.cast.temp -= #cz ntils.z.cast.temp
execute store result score #ax ntils.z.cast.temp store result score #ay ntils.z.cast.temp run scoreboard players operation #tempdist ntils.z.cast.temp *= #svz ntils.z.cast.temp
...
execute \
    ...
    if score #tempdist ntils.z.cast.temp < #hitdist ntils.z.cast.temp if function ntils:z/cast/test/with/box run data modify storage ntils:api cast.out.normal set value [0d,0d,1d]