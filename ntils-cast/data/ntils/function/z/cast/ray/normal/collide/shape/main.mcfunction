...
execute if score #vx ntils.z.cast.temp matches ..-1 run scoreboard players operation #tmi ntils.z.cast.temp >< #tma ntils.z.cast.temp
...
execute if score #vy ntils.z.cast.temp matches ..-1 run scoreboard players operation #by1 ntils.z.cast.temp >< #by2 ntils.z.cast.temp
...
execute if score #vz ntils.z.cast.temp matches ..-1 run scoreboard players operation #bz1 ntils.z.cast.temp >< #bz2 ntils.z.cast.temp
...
# as such, we compare tmi to both d (our max distance) and df (the current furthest point of intersection).
# if its less than both, then tmi is the disance to the closest intersection point of the ray and the block 
# and respects the distance requirement we set at the start
#
