...
$execute store result score #SCORE_COUNT# utils.global run scoreboard players list $(Location)
...
$data modify storage utils:z edata.get_scores set value {location:$(Location),destination:$(Destination)}
schedule function utils:z/edata/get_scores/schedule 2t