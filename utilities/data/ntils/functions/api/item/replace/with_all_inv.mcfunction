...
scoreboard players set #count ntils.int 0

function ntils:z/item/replace/with_all/gen with storage ntils:z macro
execute if data storage ntils:z item.replace.args.Ignore_Bundles run return run scoreboard players get #count ntils.int
...
return run scoreboard players get #count ntils.int